...
ARCHITECT_KEYWORDS: list[str] = [
    "architect", "system design", "rfc", "design decision",
    "migration", "race condition", "security", "cryptography",
    "ast", "parser", "distributed",
]

COMPLEX_KEYWORDS: list[str] = [
    "refactor", "optimise", "optimize", "performance",
    "concurrency", "async", "deadlock", "memory leak",
...
def record_failure(model: str) -> int:
    ...
    if len(_consecutive_failures) >= _MAX_TRACKED_MODELS and model not in _consecutive_failures:
...
def call(
    ...
    try:
        import httpx
        try:
            from runtime.provider_dispatch import build_request, infer_provider, parse_response
        except ImportError:
            import sys as _sys
            from pathlib import Path as _Path
            _sys.path.insert(0, str(_Path(__file__).resolve().parent.parent / "runtime"))
        ...
        path_suffix, headers, body = build_request(
            provider, route_result.model, messages, route_result.api_key, max_tokens, temperature
        )
        ...
        url = route_result.base_url.rstrip("/") + ("/messages" if provider == "anthropic" else path_suffix)
