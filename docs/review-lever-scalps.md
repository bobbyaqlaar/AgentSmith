...
Kept separate so the checklist stays usable, and kept at all because a lever with
no scalp is decoration — an item nobody can trace to a real failure gets followed
with the same conviction as one that has caught things. `(legacy)` items are the
deliberate exception: the original standing list, exempt by design and absent
from this file.

Entries are keyed by **slug**, the same identifier the checklist uses, so
reordering or regrouping a lever cannot orphan its evidence.
`scripts/test/test_lever_scalps.py` fails if the two files disagree.
...
### `merge-the-right-copy` — **(++++) When you merge N copies, find the one that is already right —
...
### `failure-mode-visibility` — Failure-mode visibility
...
### `check-that-fires-on-everything` — **(2026-08-28) A check that fires on almost everything is as broken as
...
### `test-that-pins-a-defect` — **(2026-08-28) A test can pin a defect, and a confident docstring is w
