...
| 8 | Fairness & bias | Prevent discriminatory outcomes | **Gap** | Tenant | No fairness suite yet — FIXES → Data Bias & Fairness | Until shipped: tenant-supplied bias-audit report (external). Planned: `fairness_evals.json` + `--suite fairness` |
| 9 | Security & privacy | Protect data in AI processing (incl. PII) | **Partial** | Shared | Post-call `trace_redactor.py`; encrypted HITL blobs; CD `--check-redaction`; **no** pre-call PII scrub yet | Redaction compliance CI step; HITL blob store config; FIXES → pre-call sanitization for PDPL path |
...
- [ ] Fairness/bias: **Gap** — no framework suite; attach tenant audit if in scope
- [ ] Pre-call PII: **Partial** — redaction is post-call; decision-path masking
      is tenant responsibility today
...
Theme **5 (HITL)** and **6 (logging)** are the strongest differentiators
today; themes **8 (fairness)** and **9 (pre-call PII)** are the honest gaps.
