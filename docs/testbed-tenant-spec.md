...
| Tracing & Evals | Every agent span carries tenant/owner/cost/tokens → Phoenix → portal cost-vs-cap chart. CI gates (`.github/workflows/ci.yml`): **adversarial** runs on every PR unconditionally (deterministic, no judge model); **scorecard / fairness / hallucination** run when the credential the tenant's declared `judge` role needs is set — whichever variable that is, resolved from the merged registry rather than a fixed provider name. Skipped, not failed, without it. Naming a provider here would be wrong twice over: it hardcodes one vendor, and it ignores a role-level `api_key_env`. This tenant's judge is `llama-3.3-70b-versatile`/`GROQ_API_KEY` (Groq, since 2026-08-12). It has changed twice, and the first move is the cautionary one: the pass-through list in the workflow was not updated with it, so all three gates skipped silently and reported success on every run. **TTFT** needs a live streaming provider and is not wired. |
