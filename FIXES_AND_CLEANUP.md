...
- name: "Validate Knowledge Graph"
  run: |
    ...
    # Compare regenerated graph against committed graph
    python3 - <<'EOF'
import json, sys
...
# map_codebase.py overwrites in place; read it back
...
- `install-ai-stack.sh` — `ai-tenant-init` injects the new step into
...
**Gap:** The RFC requirement (SPECS.md §4 Pillar 1) is enforced only by
local hooks (`pre-commit` requires ≥1 RFC in enterprise mode; `commit-msg`
requires an `RFC-NNN` reference). **CI does not re-validate this.** A PR
submitted without a corresponding RFC file in `.agent-rfc/` passes CI even in
enterprise mode, if the developer skipped or bypassed the local hook.
...
- name: "RFC gate (Pillar 1)"
  run: |
    # Only enforced when org policy is configured
...
edits a generated file directly (violating §5.3's "never edit generated
files") or if `agent-rules.yaml` is updated without re-running the hook,
the committed IDE configs silently diverge. No CI step detects this.

**Approach:**

Add a `scripts/generate-ide-config.py` script (Phase 5 item in SPECS.md §22
that already references this) that regenerates IDE configs from
`agent-rules.yaml` and diffs against the committed files. Add to CI:

```yaml
- name: "IDE config drift check (Pillar 6/7)"
  run: |
    python3 scripts/generate-ide-config.py --check-only
    # Exits 0 if committed files match what agent-rules.yaml would generate.
    # Exits 1 with diff if they've drifted.
  continue-on-error: true   # warn only — blocking is a Phase 5 decision
```

This closes the Phase 5 item: "templates/agent-rules.yaml single-source IDE
config generation" — the generator script is the missing link between the
YAML spec and CI enforcement.
...
- `install-ai-stack.sh` — hook calls `generate-ide-config.py` instead of
  inline heredoc templating (Phase 5 extraction)
...
doesn't validate Phoenix reachability, hook integrity, identity env vars, or
unresolved MAJOR/CRITICAL issues before a deploy.

**Note:** The CD workflows already run `--check-redaction` (SPECS.md §27
compliance), so this is a partial gap, not a complete absence.

**Approach:**

Add a lightweight `verify_system.py` call to the CI workflows — non-blocking,
reporting only, so greenfield projects without Phoenix configured don't fail:
...
This surfaces identity gaps and Phoenix misconfig in the PR log without
blocking PRs on missing secrets.
...
**Merge order:** P10a and P10b can be done in one PR (both are CI step
additions). P10c requires `generate-ide-config.py` first. P10d is a 3-line
addition to each CI template — lowest friction, do it first as a warm-up.
...
What follows is the two pieces it does **not** cover.
...
in the repo. The Knowledge Graph is structured lookup, not embedding
similarity — it does not satisfy (b) below.
