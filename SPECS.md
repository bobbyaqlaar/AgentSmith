...
- IDE rules and guardrails for Cursor, Claude Code, Codex, Gemini CLI, Copilot and Antigravity (`CLAUDE.md`, `.cursorrules`, `AGENTS.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.agents/skills/`)
...
- This constraint is injected into `.cursorrules`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.github/copilot-instructions.md`, and Antigravity skills. The single-source template is `templates/agent-rules.yaml`; IDE-specific files are generated from it by the `post-checkout` hook.
...
Rules are generated from `templates/agent-rules.yaml` and written to: `.cursorrules` (Cursor), `CLAUDE.md` (Claude Code), `AGENTS.md` (Codex), `GEMINI.md` (Gemini CLI), `.github/copilot-instructions.md` (Copilot), and `.agents/skills/*/skill.md` (Antigravity). All are covered by `--check-only`.
...
| File | Contents |
|---|---|
| `.cursorrules` | 14-pillar system rules, stack addendum, OTLP endpoint |
...
| `runtime/version.py` | `framework_version()` — the running framework's version, for the OTel Resource and the run-status POST. `warn_if_declared_version_differs()` warns once at worker startup when it disagrees with `framework.version` in tenant.yaml, which is a declaration nothing installs from. Compared at MINOR granularity, the granularity the compatibility matrix is written at. Needs no git, tags or checkout — the only such check available to a tenant in a separate repository. An installed release reports bare; a source checkout reports `x.y.z+src`, because a working copy's pyproject version says which release it descends from and nothing about what it contains. |
...
Six targets, one source. Laid out by tool rather than by feature, because the
column-per-tool shape stopped scaling once there were more than three.

| Tool | File | Shape | Why this shape |
|---|---|---|---|
| Cursor | `.cursorrules` | Numbered markdown sections + stack addendum | Read once per session; the original template |
| Claude Code | `CLAUDE.md` | Short, with a session-start checklist | Points at `.cursorrules` for detail — Claude Code reliably opens neighbouring files |
| Codex | `AGENTS.md` | Self-contained, full pillar text | The cross-tool convention. Self-contained because an agent ingesting only this file would otherwise get a rule file consisting of one redirect |
| Gemini CLI | `GEMINI.md` | As `AGENTS.md`, plus a tool-name mapping note | Read once at session start, so the reasoning earns its tokens |
| Copilot | `.github/copilot-instructions.md` | Condensed — one imperative per pillar | Prepended to every *request*, so length is a recurring cost; the reasoning cannot survive that budget and lives in `AGENTS.md` |
| Antigravity | `.agents/skills/*/skill.md` | Multiple composable skill files | Grouped by trigger rather than by rule |

All six are generated from `templates/agent-rules.yaml` by the `post-checkout` hook, and all six are covered by `--check-only`. Existing files are not overwritten, preserving project-level customisations.
...
│   │                            #   .agents/skills/), not in this directory
